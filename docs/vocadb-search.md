...
VocaDB はボランティア運営のコミュニティ DB で、公開された流量上限は見当たらないが
「上限が書かれていない」だけで無制限ではない。素性の分かる User-Agent
(`setnote (+https://setnote.yu-web.site)`) を名乗ってリクエストしている。

流量で弾かれた場合はエラーになり、ユーザーには「VocaDBの検索に失敗しました」が出る。
黙って別の結果が返ることはない。
